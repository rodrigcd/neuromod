...
def main(argv):
    ...
    parser.add_argument(
        '--save-path', type=str, default="../results/computation/")
    parser.add_argument(
        '--dataset', type=str, default="MNIST")
    parser.add_argument(
        '--n-steps', type=int, default=6000)
    parser.add_argument(
        '--iter-control', type=int, default=50)
    ...
    approx_solver = LinearNetEq(**equation_params)
    approx_control = LinearNetControl(**control_params)
    ...
    approx_cumulated_reward = []
    for i in tqdm(range(iter_control)):
        R = approx_control.train_step(get_numpy=True, dataset_sampler=dataset)
        approx_cumulated_reward.append(R)

    plt.plot(cumulated_reward, label="equation")
    plt.plot(approx_cumulated_reward, label="batch approximation")
    plt.legend()
    plt.show()
