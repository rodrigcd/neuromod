...
class SingleLayerNet(object):
    ...
    def __init__(self, input_dim, output_dim, learning_rate=1e-5, W0=None, reg_coef=0.1, intrinsic_noise=1.0,
                 init_zero=False):
        ...
        if W0 is None:
            if init_zero:
                self.W0 = torch.zeros(size=(self.output_dim, self.input_dim), requires_grad=False,
                                      device=self.device, dtype=self.dtype)
            else:
                self.W0 = torch.normal(mean=0, std=0.01, size=(self.output_dim, self.input_dim),
                                       requires_grad=False, device=self.device, dtype=self.dtype)
            #self.W0 = torch.from_numpy(np.array([[0.02, 0.01],
            #                                     [-0.01, -0.02]])).requires_grad_(False).type(self.dtype).to(self.device)
        else:
            self.W0 = torch.from_numpy(W0).to(self.device)
        self.W = self.W0.clone().detach().requires_grad_(True)

    def forward(self, x):
        x_tensor = torch.from_numpy(x.T).type(self.dtype).to(self.device)
        y_hat = torch.matmul(self.W, x_tensor)
        return y_hat + torch.normal(mean=0, std=self.intrinsic_noise, size=y_hat.shape).to(self.device)

    def train_step(self, x, y):
        y_pred = self.forward(x)
        y_target = torch.from_numpy(y.T).to(self.device)[:self.output_dim, :]
